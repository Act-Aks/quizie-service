...
fun RequestValidationConfig.validateQuizTopic() {
    validate<QuizTopic> { quizTopic ->
        when {
            quizTopic.name.isBlank() || quizTopic.name.length < 3 -> ValidationResult.Invalid(
                reason = "Topic must be at least 3 characters long and not empty"
            )

            quizTopic.imageUrl.isBlank() -> ValidationResult.Invalid(
